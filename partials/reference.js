...
Reference.prototype.bindEvt = function bindEvt () {
	this.input.addEventListener('blur', function (e) {
		...
		var blocks = this.el.parentNode.parentNode.parentNode.querySelectorAll('.list-block');
		var datas = [];
		var saveJson = [];
		var key;
		...
		if(typeof blocks !== 'undefined' && blocks !== null && blocks.length > 0){
			Array.prototype.forEach.call(blocks, function (block) {
		  	...
		  	Array.prototype.forEach.call(inputs, function (input) {
		  		...
		  		if(typeof datas[key][index] === 'undefined' || datas[key][index] === null) {
		  			datas[key][index] = {};
		  		}
		...
		xhr.open(
			'GET', '/abe/plugin/abe-reference/create?&k=' + encodeURIComponent(this.labelText) + 
																							'&v=' + JSON.stringify(saveJson) + 
																							'&f=' + key
		);
		xhr.send(null);
